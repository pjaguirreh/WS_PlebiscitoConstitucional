...
vot_comunas <- tibble(com_cod = rep(0, length(lista_comunas)), 
                      ...
                      vot_apruebo = rep(0, length(lista_comunas)), 
                      vot_rechazo = rep(0, length(lista_comunas)), 
...
for (i in seq_along(lista_comunas)){
  ...
  # datos de votación
  detalle_comuna <- fromJSON(file = paste0("https://servelelecciones.cl/data/elecciones_constitucion/computo/comunas/",
                                           lista_comunas[[i]]$c,
                                           ".json"))
